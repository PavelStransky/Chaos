...
\section{Integrable systems}\label{sec:Integrable}
    ...
    \subsection{Quantum integrable systems}
        Again, in analogy with classical physics, when there exist $d=f$ independent quantum integrals of motion, the system is called \emph{integrable}.
    ...
    \subsection{Nearest neighbour spacing distribution}
        ...
        It measures the probability that we find two neighbouring levels one at a distance $s$ from the other.

        Let us derive now what the Nearest Neighbour Spacing Distribution (NNSD) of a generic quantum integrable system looks like.
        Suppose there is a quantum level at $E=E_{0}$, and $p(s)$ is the probability density that we find the next level at distance $s$ from $E_{0}$.  
        ...
        Number $p(s)\d s$ is therefore the probability that we find the level neigbouring to $E_{0}$ at interval $\d s$.
        ...
        Why? 
        The reason is that these systems are what is sometimes called \emph{superintegrable}: Their symmetry is higher than necessary, and they have more integrals of motion than $f=3$ (for the hydrogen atom it is the Runge-Lenz vector; isotropic harmonic oscillator has an analogous additional integral of motion).
        ...
        \begin{task}\label{task:2Dbox}
            ...
            It is an integrable system because it is fully separable (it can be solved in each dimension separately and the appropriate Hilbert space for the full system can be written as a product of Hilbert spaces of the subsystems; the Hamiltonian of each of the subsystems is an integral of motion on its own).
            ...
            Calculate a sufficient and complete part of the spectrum of this system (at least $1000$ lowest-lying states, but it is easy and fast to reach up to $10^6$ states) and plot a histogram of the nearest neighbour spacing distribution.
            ...
            Play a little with this simple model, you can, for example, use a rational number for $\alpha$ and test how sensitive is the NNSD to ``resonances'' in the spectrum caused by the commensurate box dimensions.
        ...
        \begin{solution}
            ...
            \begin{itemize}
                \item 
                    ...
                    It contains functions to calculate the spectrum and renormalize it to obtain unit average level density.
                \item
...
\section{Chaotic systems}
    ...
    \subsection{Wigner surmise}\label{sec:Wigner}
        ...
        At that time it was observed in the measurements of high-lying slow nuclear resonances a lack of close states.
        At first, it was being explained by imperfect measuring techniques, but then Wigner~\cite{Wig51,Wig55,Wig57} proposed and succeeded to use the Random Matrix Theory (RMT) to describe this suspicious behaviour of the resonances, and he formulated the so-called
        \begin{theorem}
            {\bf (Wigner surmise)}
...
\section{Gaussian random matrix ensembles}
    ...
    The development of the random matrix theory began in the 1920s~\cite{Wis28} but did not draw much attention at that time.
    ...
    We picture a complex nucleus as a `black box' in which a large number of particles are interacting according to unknown laws.
    ...
    \begin{enumerate}
        \item {\bf Gaussian Orthogonal Ensemble ($\ensemble{GOE}$), $\beta=1$}: 
            ...
            The majority of systems belong to the $\ensemble{GOE}$.
        ...
        \item {\bf Gaussian Symplectic Ensemble ($\ensemble{GSE}$), $\beta=4$}:
            ...
            Systems in this class exhibit Kramers's degeneracy.
