...
\def\remember#1{{\color{ForestGreen}\bf{#1}}}
...
\section{Literature}
\begin{itemize}
    \item \cite{Gutzwiller1990}~Martin C. Gutzwiller, {\it Chaos in Classical and Quantum Mechanics} (Springer, 1990)
        \begin{itemize}
    ...
    \item \cite{Haake2010}~Fritz Haake, {\it Quantum Signatures of Chaos} (Springer, 2010)
        \begin{itemize}
            \item Up-to-date topics, including chaotic dissipative systems and supersymmetric approaches.
        \end{itemize}

    \item \cite{Bohigas1991}~Oriol Bohigas, {\it Random Matrix Theories and Chaotic Dynamics}, Les Houches LII, ed. M.-J. Gianonni, A. Voros, J. Zinn-Justin, 1989
        \begin{itemize}
            \item Brief and concise notes on the basics of quantum chaos.
        \end{itemize}

    \item \cite{Mehta2004}~Madan L. Mehta, {\it Random Matrices} (Elsevier 2004).
        \begin{itemize}
            ...
            \item \remember{Semiclassical quantization}
                ...
                \item \remember{Spectral correlations}
                    \begin{itemize}
                        ...
                        \item The most important mathematical tool here is the \remember{Random matrix theory}~\cite{Mehta2004}.
                        \item Spectral correlations are sometimes even used to define chaos in quantum mechanics.How\-ever, the connection between this definition and the ones based on semiclassical approaches is often loose; therefore, Michael Berry proposes to use the term Quantum Chaology instead of Quantum Chaos~\cite{Berry1989}.
                        \item The mostly cited paper conjecturing a connection between classical chaos and correlations in quantum spectra is the one by Bohigas, Giannoni and Schmit~\cite{Bohigas1984}.
                    \end{itemize}
...
\section{Integrable systems}\label{sec:Integrable}
    \subsection{Quantum integrals of motion}
        ...
        Suppose we have a quantum system with $f$ degrees of freedom described by a time-independent Hamiltonian $\operator{H}$.
        ...
        \begin{subequations}
            ...
            \end{align}                
        \end{subequations}
        are the so called \emph{quantum integrals of motion}.\footnote{
            A common example of such integrals of motion is the angular momentum and parity, $J^{\pi}$.
        }
    ...
    \subsection{Quantum integrable systems}
        ...
        The result of the previous section implies that in such a system, each state can be uniquely labelled with a set of $f$ quantum numbers that are eigenvalues of the $f$ integrals of motion.
        The typical eigenstates of these operators form a basis for the Hamiltonian, in which the Hamiltonian has a diagonal form.
        Therefore, \remember{in quantum integrable systems, all the eigenstates are uncorrelated.}

    \subsection{Einstein-Brillouin-Keller (EBK) quantization}
        If a classical system is integrable, its Hamiltonian can be expressed in the \emph{action-angle coordinates} $(J_{j},\phi_{j}),j=1,\dotsc,f$ and is independent of the angles $\phi_{j}$, $H=H(\vector{J})$.
        This implies that the classical motion is confined to $f$-dimensional tori in the phase space, and the classical dynamics is given by the Hamilton equations of motion
        \begin{subequations}
            \begin{align}
                \derivative{J_{j}}{t}&=\partialderivative{H}{\phi_{j}}=0,\\
                \derivative{\phi_{j}}{t}&=\partialderivative{H}{J_{j}}\equiv\omega_{j},
            \end{align}                
        \end{subequations}
        with a simple solution
        \begin{subequations}
            \begin{align}
                \phi_{j}(t)&=\omega_{j}t+\phi_{j}(0),\\
                J_{j}(t)&=J_{j}(0)=\const.
            \end{align}                
        \end{subequations}
        EBK then states that each of the action coordinates can be quantized separately,
        \begin{equation}
            J_{j}=\hbar n_{j}+a_{j},
        \end{equation}
        where $n_{j}\in\mathbb{N}$ is a quantum number and $a_{j}$ a real constant.
        This result supports the conclusion of the previous section.
    ...
    \subsection{Nearest neighbour spacing distribution}
        ...
        Number $p(s)\d s$ is the probability that we find a level neighbouring to $E_{0}$ at interval $\d s$.
        ...
        \begin{task}\label{task:2Dbox}
            ...
            A detailed discussion of this system (concluding that the levels in the infinite well are, in fact, correlated) is given in Ref.~\cite{Casati1985}.
        \end{task}
        ...
        \begin{solution}
            ...
            \begin{itemize}
                \item 
                    ...
                    It contains functions to calculate the energy spectrum and the smooth part of the level density.

                \item
                    \file{statistics.py}: Functions to unfold the spectrum (more about the unfolding in Section~\ref{sec:Unfolding}) and obtain unit average level density; functions to calculate the nearest neighbour spacing distribution of the unfolded spectrum and the Poisson and Wigner distribution functions.
                \item                    
                    \file{plots.py}: Functions that plot histograms and compare numerically obtained distributions with exact ones.
            \end{itemize}
...
\section{Chaotic systems}
    ...
    For more details, I strongly recommend reading the first chapter of Mehta's monography~\cite{Mehta2004}.
    ...
    \subsection{Wigner surmise}\label{sec:Wigner}
        ...
        At first, it was being explained by imperfect measuring techniques, but then Wigner~\cite{Wigner1951,Wigner1955,Wigner1957} proposed and succeeded in using the Random Matrix Theory (RMT) to describe this suspicious behaviour of the resonances, and he formulated the so-called
        \begin{theorem}
            {\bf (Wigner surmise)}
        ...
        But then, in 1984, Bohigas, Giannoni and Schmit published a ground-breaking paper~\cite{Bohigas1984} stating:
        \begin{theorem}
            ...
            On the contrary, quantum systems with stable classical analogues (not necessarily integrable) have uncorrelated energy levels~\cite{Berry1997}.
        \end{theorem}
        ...
        \begin{enumerate}
            \item Until now, the conjecture has not been proved in its generality.\footnote{
                    ...
                    \end{equation*}
                    The complete discussion of this classification is beyond the scope of this lecture (and it belongs to the area of classical chaos theory); you can find it in~\cite{Sinaj1977}.
                    It has been proved, though, that mixing at the classical level is sufficient to generate random matrix statistics.
                } 
                However, there is a vast list of numerical studies based on a wide variety of systems that support its validity (but there are also several counter-examples of systems that are classically regular but have random-matrix-like quantum spectrum,\footnote{For example the Šeba's billiard~\cite{Seba1990}: a circular billiard with a $\delta$-function sitting in the middle.} and vice versa).
        \end{enumerate}
...
\section{Gaussian random matrix ensembles}
    ...
    The development of the random matrix theory began in the 1920s~\cite{Wishart1928} but did not draw much attention then.
    The theory became more popular when Wigner formulated his surmise, see Section~\ref{sec:Wigner},
    but mathematically rigorous foundations of the so-called Random Matrix Theory (RMT) were established in a series of beautiful papers by Dyson~\cite{Dyson1962,Dyson1962a,Dyson1962b}\footnote{
    ...
    \begin{enumerate}
        ...
        \item {\bf Gaussian Symplectic Ensemble ($\ensemble{GSE}$), $\beta=4$}:
            ...
            Note that the first experiment to obtain the $\ensemble{GSE}$-like spectrum was performed quite recently~\cite{Rehemanjiang2016} using specially designed quantum graphs.
    \end{enumerate} 
    ...
    \begin{itemize}
        \item Altland-Zirnbauer universality classes~\cite{Altland1997} (7 additional classes by relaxing Equation~\eqref{eq:PHInvariance}, taking into account particle-hole symmetry or chiral symmetry).
        ...
        \item Ginibre ensembles~\cite{Ginibre1965} (condition~\eqref{eq:PHInvariance} completely disregarded).
    \end{itemize} 
        ...
        \subsubsection{Nearest neighbour spacing distribution}
            ...
            \begin{figure}[!htbp]
                ...
                \caption{
                    ...
                    This panel is taken from~\cite{Haake2010}.
                }	
                \label{fig:Ensembles}
            \end{figure}
    ...
    \subsection{Wigner semicircle law}
        ...
        converges to the \remember{Wigner semicircle law}~\cite{Wigner1957}
        \begin{equation}
            \label{eq:Semicircle}
            \boxed{
...
\section{Statistical analysis of a quantum spectrum}
    ...
    \subsection{Level density}
        ...
        It is system dependent, expressed by the semiclassical \remember{Weyl fomula}\footnote{
            It is worth mentioning a related work by Mark Kac, discussing general properties of a vibrational spectrum of a ``drum'': a membrane of an arbitrary shape with a fixed boundary~\cite{Kac1966}.
        }
        \begin{equation}
            \label{eq:Smooth}
        ...
        The oscillatory part carries information about the spectral correlations.
        It is given by the \emph{Gutzwiller trace formula}~\cite{Gutzwiller1990}
        \begin{equation}
            \label{eq:Gutzwiller}
            \tilde{\rho}(E)=\sum_{p}\sum_{r=1}^{\infty}\frac{rT_{p}}{\sqrt{\norm{\det\left(\unit{1}-\matrix{M}_{p}\right)}}}\cos\left[\frac{1}{\hbar}r S_{p}(E)-\frac{\pi}{2}r\nu_{p}\right]
        \end{equation}
        where $p$ indexes all the simple periodic orbits of the system, $r$ specifies the number of repetitions, $T_{p}$ is the period of the orbit, $\matrix{M}_{p}$ is the stability (monodromy) matrix, $S_{p}(E)$ is the action of the orbit, and $\nu_{p}$ is a phase connected with the orbit (the \emph{Maslov index}).
    ...
    \subsection{Brody distribution}
        ...
        It has been conjectured~\cite{Percival1973} (\emph{Percival conjecture}) that the parts of the quantum spectrum that correspond to the remnants of classical tori (regions with classical stable dynamics) are statistically independent of the parts related with the chaotic classical regions, so that the energy level sequence is a mixture of levels with Wigner and with Poissonian statistics.
        Mathematically it is described by the \emph{Berry-Robnik distribution}~\cite{Berry1984a}.
        However, in practice, it is much more common to use the \emph{Brody distribution}~\cite{Brody1973,Brody1981}, which is, in fact, an artificial distribution obtained by assuming level repulsion
        \begin{equation}
            \mu(s)=s^{\omega}
        \end{equation}
        ...
        Despite having known conflicts with theoretical predictions for the NNSD behaviour, Brody distribution is highly successful and often gives better estimate of the degree of system's chaoticity than other more firmly founded theoretical distributions.\footnote{
            Apart from the Berry-Robnik distribution, there is also Izrailev distribution~\cite{Izrailev1989}, Caurier-Grammaticos-Ramani distribution and Lenz-Haake distribution on the market.
    ...
    \subsection{Long-range correlations}
        ...
        \subsubsection{$\Delta_{3}$ statistics}
            ...
            For discrete set of unfolded levels $e_{j}$ there exists a practical formula to calculate $\Delta_{3}$~\cite{Bohigas1975},
            \begin{align}
                \Delta_{3}(L)
            ...
            \begin{equation}
                \Delta_{3}(L)=\left\{\begin{array}{ll}
                    ...
                    \frac{1}{\pi^{2}\beta}\left(\ln L+c_{\beta}\right)+\mathcal{O}(L^{-1}) & \text{for spectrum of Gaussian random matrices},
                \end{array}\right.
                \label{eq:Delta3}
            \end{equation}
        ...
        \subsubsection{$\Sigma^{2}$ statistics}
            ...
            \begin{equation}
                \sigma^{2}(E_{0};L)
                    ...
                    =\left\langle\left[N\left(E_{0}+\frac{L}{2}\right)-N\left(E_{0}-\frac{L}{2}\right)-L\right]^{2}\right\rangle,
                \label{eq:Sigma2}
            \end{equation}  
            ...
            \begin{equation}
                \Sigma^{2}(L)=\left\{\begin{array}{ll}
                    ...
                    \frac{2}{\pi^{2}\beta}\left(\ln L+d_{\beta}\right)+\mathcal{O}(L^{-1}) & \text{for spectrum of Gaussian random matrices},
                \end{array}\right.
            \end{equation}
            where
            \begin{equation}
